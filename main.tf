...
data "archive_file" "lambda_fibonacci" {
  type = "zip"

  source_dir  = "${path.module}/fibonacci"
  output_path = "${path.module}/fibonacci.zip"
}
...
resource "aws_s3_object" "lambda_fibonacci" {
  bucket = aws_s3_bucket.lambda_bucket.id

  key    = "fibonacci.zip"
  source = data.archive_file.lambda_fibonacci.output_path

  etag = filemd5(data.archive_file.lambda_fibonacci.output_path)
}
...
resource "aws_lambda_function" "fibonacci" {
  ...
  s3_bucket = aws_s3_bucket.lambda_bucket.id
  s3_key    = aws_s3_object.lambda_fibonacci.key

  runtime = "nodejs14.x"
  handler = "fibonacci.handler"

  source_code_hash = data.archive_file.lambda_fibonacci.output_base64sha256
...
resource "aws_cloudwatch_log_group" "fibonacci" {
  name = "/aws/lambda/${aws_lambda_function.fibonacci.function_name}"
...
resource "aws_apigatewayv2_integration" "fibonacci" {
  api_id = aws_apigatewayv2_api.lambda_api.id

  integration_type = "AWS_PROXY"
  integration_method = "POST"
  integration_uri  = aws_lambda_function.fibonacci.invoke_arn
}
...
resource "aws_apigatewayv2_route" "fibonacci" {
  api_id = aws_apigatewayv2_api.lambda_api.id

  route_key = "GET /fibonacci"
  target    = "integrations/${aws_apigatewayv2_integration.fibonacci.id}" 
}
...
resource "aws_lambda_permission" "api_gw_fibonacci" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.fibonacci.function_name
  principal     = "apigateway.amazonaws.com"
